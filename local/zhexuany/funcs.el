...
(defun zhexuanyinsert-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
  (insert (zhexuany/retrieve-chrome-current-tab-url)))

(defun zhexuany/retrieve-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
...
(defun zhexuany/org-archive-done-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun zhexuany/org-archive-cancel-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun zhexuany/single-lines-only ()
...
(defun zhexuany/comment-box (b e)
...
(defun zhexuany/rename-file-and-buffer ()
...
(defun zhexuany/word-count-for-chinese ()
  "「較精確地」統計中/日/英文字數。
- 文章中的註解不算在字數內。
...
;; (defun zhexuany/open-line-above()
...
;; (defun zhexuany/open-line-below()
...
(defun zhexuany/yank-to-end-of-line ()
  "Yank to end of line."
  (interactive)
  (evil-yank (point) (point-at-eol)))

(defun zhexuany/evil-quick-replace (beg end )
  (interactive "r")
...
(defun zhexuany/run-current-file ()
  ...
  (let* (
    ...
    (if (string-equal ξfSuffix "el") ; special case for emacs lisp
      ...
      (if ξprog-name
          (progn
            (message "Running…")
            (async-shell-command ξcmd-str "*zhexuany/run-current-file output*"))
        (message "No recognized program file suffix for this file.")))))

;; "http://xuchunyang.me/Opening-iTerm-From-an-Emacs-Buffer/"
(defun zhexuany/iterm-shell-command (command &optional prefix)
  "cd to `default-directory' then run COMMAND in iTerm.
With PREFIX, cd to project root."
  (interactive (list (read-shell-command
                      "iTerm Shell Command: ")
                     current-prefix-arg))
  (let* ((dir (if prefix (zhexuany/project-root)
                default-directory))
...
(defun zhexuany/browser-refresh--chrome-applescript ()
  (interactive)
  (do-applescript
   (format
    "
...
(defun zhexuany/open-file-with-projectile-or-lsgit ()
  (interactive)
  (if (zhexuany/project-root)
      (counsel-git)
    (helm-projectile-find-file)))

;; http://blog.lojic.com/2009/08/06/send-growl-notifications-from-carbon-emacs-on-osx/
(defun zhexuany/growl-notification (title message &optional sticky)
...
(defun zhexuany/growl-timer (minutes message)
  ...
  (run-at-time (* (string-to-number minutes) 60)
               ...
               (lambda (minute message)
                 (zhexuany/growl-notification "Emacs Reminder" message t))
               minutes
               message))

(defun zhexuany/goto-match-paren (arg)
